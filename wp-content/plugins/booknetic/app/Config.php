...
class Config
{
	...
	public static function registerTextDomain()
	{
		add_action( 'plugins_loaded', function()
		{
			...
			if( Helper::isSaaSVersion() && ! Permission::isSuperAdministrator() && Permission::tenantId() > 0 && file_exists( WP_PLUGIN_DIR . '/' . $path . '/' . Permission::tenantId() . '/booknetic-' . get_locale() . '.mo' ) )
			{
    ...
    public static function registerCoreWorkflowEvents()
    {
        ...
        {
            if( ! empty( $customerId ) && ! empty( $customerPassword ) )
            {
