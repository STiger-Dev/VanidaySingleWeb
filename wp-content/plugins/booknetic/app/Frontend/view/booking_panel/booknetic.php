...
<div class="booknetic_appointment<?php echo Helper::isRTLLanguage( Permission::tenantId() ) ? " rtl": "" ?>" id="booknetic_theme_<?php echo $theme_id;?>">
	<div class="booknetic_appointment_steps <?php echo Capabilities::tenantCan( 'upload_logo_to_booking_panel' ) && Helper::getOption('display_logo_on_booking_panel', 'off') == 'on' ? 'has-logo' : ''; ?>">
		...
		<div class="booknetic_appointment_steps_body">
			...
			foreach ( $steps_order AS $stepId )
			{
				...
				?>
				<div class="booknetic_appointment_step_element<?php echo $step['hidden'] ? ' booknetic_menu_hidden' : ''?>" <?php echo !empty($step['value']) ? ' data-value="'.$step['value'].'"' : ''?> data-step-id="<?php echo $stepId?>" data-loader="<?php echo $step['loader']?>" data-title="<?php echo $step['head_title']?>" <?php echo isset($step['attrs']) && !empty($step['attrs']) ? $step['attrs'] : ''?>>
	...
	<?php if( Helper::isSaaSVersion() && !( Permission::tenantInf() && Capabilities::tenantCan('remove_branding') && Helper::getOption('remove_branding', 'off') == 'on' ) ):?>
